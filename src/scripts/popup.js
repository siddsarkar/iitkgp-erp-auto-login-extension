...
function updateUI(restoredSettings) {
  ...
  if (!authCredentials.dark) {
    ...
    hIcon.setAttribute(
      "src",
      browser.runtime.getURL("/pages/assets/icons/header_icon_light.png")
    );
...
async function getQuestions(cb) {
  ...
  let q = await res.text(); // Question or FALSE
...
function resetHandler() {
  let actionBtnYes = document.createElement("div");
  actionBtnYes.className = "action";
  actionBtnYes.textContent = "Yes";
  let actionBtnCancel = document.createElement("div");
...
function toggleDark() {
  if (document.body.classList.contains("dark")) {
    ...
    hIcon.setAttribute(
      "src",
      browser.runtime.getURL("/pages/assets/icons/header_icon.png")
    );
  } else {
    themeInfo.textContent = "on";
...
function handleMessageResponse(response) {
  if (response.action === "update_check") {
    ...
    if (update) {
      ...
      let whatsnew = document.createElement("p");
